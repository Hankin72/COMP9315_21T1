...
void showALL(uint32 b, int *starList, int start, int length) {
	...
	int idx = starList[start];
	// 0 
	uint32 b0 = unsetBit(b, idx);
	showALL(b0, starList, start + 1, length);

	// 1
	uint32 b1 = setBit(b, idx);
	showALL(b1, starList, start + 1, length);
}
...
int main(int argc, char **argv)
{
	...
	while (fgets(line,200,stdin) != NULL) {
		...
		if (!parseQuery(line)) {
			...
			uint32 mah = 0;
			int *starList = calloc(MAX_CVLEN, sizeof(int));
			int length = 0;
			for (int i = ncv -1; i >= 0; i--) {
				// printf("%d  att : %d\n", cv[i].attr);
				// printf("%d  bit : %d\n", cv[i].bit);
				// printf("%d  hash: ");
				int attr = cv[i].attr;
				int position = cv[i].bit;
				uint32 hash = q[attr].hash;
				int isKnown = q[attr].known;

				if (isKnown == 0) {
					starList[length++] = i;
				} else if (bitIsSet(hash, position)) {
					mah = setBit(mah, i);
				}
			}

			printf("pages:");
			showALL(mah, starList, 0, length);
			printf("\n");
			free(starList);
		}
		printf("query> ");
