...
// - if looked enough and there are no available slots, return NONE
...
int findVictim(BufPool pool) {
    ...
    // MAX_USAGE =3
    uppers = buffers * MAX_USAGE;
    index = pool->clock;

    do {
        index = index % buffers;
        pinCount = pool->bufs[index].pin;
        popularity = pool->bufs[index].pop;

        if (pinCount == 0 && popularity == 0) {
            slot = index;
            break;
        }
        if (popularity > 0) {
            pool->bufs[index].pop = pool->bufs[index].pop - 1;
        }
        index++;
        //
        attps++;
        pool->clock++;
        pool->clock = pool->clock % buffers;
    }while(slot == NONE && attps <= uppers);
