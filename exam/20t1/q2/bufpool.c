...
int findVictim(BufPool pool)
{
    ...
    int attempts=0;
    int slot_index = pool->clock;

//    buffer = 5
    while(slot == NONE && attempts< pool->nbufs * MAX_USAGE){
        slot_index = slot_index % pool->nbufs;

        if (pool->bufs[slot_index].pin ==0 && pool->bufs[slot_index].pop==0){
            slot = slot_index;
            break;
        }
        if (pool->bufs[slot_index].pop>0){
            pool->bufs[slot_index].pop--;
        }
        slot_index ++;
        // add
        attempts++;
        pool->clock++;
        pool->clock = pool->clock % pool->nbufs;
    }
    ...
    if (pool->bufs[slot_index].dirty>0){
        pool->nwrites++;
    }
